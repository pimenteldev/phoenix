...
function index() {
  ...
  useEffect(() => {
    fetch(`${URL_API}get_informe_ventas_dia.php`, requestOptions)
      ...
      {viewOrders ? (
        <>
          <div className={style.title_report}>
            Informe de Ventas Día<small>{useDateTimeFormat(Date.now())}</small>
          </div>
                    ...
                    {ordersFilter.map((order, index) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className={style.center}>{parseFloat(order.order_price_dolar).toFixed(2)}Bs</td>
